...
public class PayCtl {
	...
	// /** 礼包 type:新手的2礼包 还是非新手的-2礼包 还是所有礼包 */
	// @ResponseBody
	// @RequestMapping("/giftBag")
	// public ReMsg getGiftBag(String payType, Integer type, HttpServletRequest
	// req) {
	// return new ReMsg(ReCode.OK, GiftBagService.getGifgBag(payType, type));
	// }

	/** 是否弹出购买新手礼包界面 */
	@ResponseBody
	@RequestMapping("/noviceGiftBag")
	public ReMsg noviceGiftBag(HttpServletRequest req) {
		return orderService.noviceGiftBag();
	}
	...
	public void weixinPay(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String s = weiXinPayService.callback(context);
	...
	public void alipay(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		String ret = aliPayService.alipayCallback(request, response);
