...
public class ImageUtils {
	...
	public static void main(String[] args) {
		// 1-缩放图像：
		// 方法一：按比例缩放
		ImageUtils.scale("e:/abc.jpg", "e:/abc_scale.jpg", 2.0f);// 测试OK
		// 方法二：按高度和宽度缩放
	 ...
	 *            期望最大宽度
	 */
	public final static void decrease(String srcImageFile, String result,
		...
		try {
			...
			// 如果图片是纵向，并且源图片大小比期望值要大(参数的宽高要对调一下)
					...
					&& (srcWidth > height || srcHeight > width)) {
				int temp = height;
				height = width;
				width = temp;
				scale = Float.valueOf(width) / Float.valueOf(srcWidth);
