...
//		}
	...
	public String singleSend(String dst, String msg) throws UnsupportedEncodingException {
		...
		/*
		 * sUrl = "http://www.139000.com/send/gsend.asp?name=" + comName +
		 * "&pwd=" + comPwd + "&dst=" + dst + "&msg=" + msg;
		 */
...
//		SmsService sms = new SmsService(); // 这里修改成你自己的用户名和密码
//		String content = "注册验证码为：" + random(4) + ",如非本人操作，请忽略！";
//		String str = "";
//		try {
//			str = sms.singleSend("13718780428", content);
...
//		} // 这里修改成要发送的手机号码和发送内容
		...
		//System.out.println(isChinaPhoneLegal("14610001000"));
		// String newstring = null;
		// try {
		// System.out.println(str);
		// newstring = new String(str.getBytes("GBK"), "utf-8");
		// System.out.println("aaa:"+newstring);
		// } catch (Exception ex) {
	...
	// }
	//
	// public ReMsg sendAuthCode(HttpServletRequest req, Long phone)
	// throws UnsupportedEncodingException {
	// String ip = super.getIp(req);
	// ValueOperations<String, String> opsv = getRedisTemplate().opsForValue();
	//
	...
	//
	// Long count = opsv.increment(RK.smsPhone(phone), 1);
	// if (24 * 3600 - getRedisTemplate().getExpire(RK.smsPhone(phone),
	// TimeUnit.SECONDS) < 60) {
	// return new ReMsg(ReCode.SMS_PHONE_TOO_OFTEN_60);
	// }
	//
	...
	//
	// String authCode = random(4);
	// SmsLog sl = new SmsLog(super.getNextId(DocName.SMS_LOG), phone,
	// authCode);
	// super.getMongoTemplate().save(sl);
	// String res = this.singleSend("" + phone, "您的验证码为：" + authCode
	// + ",如非本人操作，请忽略！");
	// // System.out.println(res);
	// if (res.contains("发送成功")) {
	// getRedisTemplate().expire(RK.smsIp(ip), 1, TimeUnit.DAYS);
	// getRedisTemplate().expire(RK.smsPhone(phone), 1, TimeUnit.DAYS);
	// return new ReMsg(ReCode.OK);
	// } else {
	...
	// }
	//
	// public boolean validCode(Long phone, String code)
	// throws JsonParseException, JsonMappingException, IOException {
	// List<DBObject> codes = super.getC(DocName.SMS_LOG)
	// .find(new BasicDBObject("phoneNo", phone).append("code", code))
	// .sort(new BasicDBObject("_id", -1)).limit(1).toArray();
	// if (codes.size() == 1) {
	// DBObject dbo = codes.get(0);
	// Integer status = DboUtil.getInt(dbo, "status");
	// Long updateTime = DboUtil.getLong(dbo, "updateTime");
	// if (status == Const.STATUS_DEF
	// && System.currentTimeMillis() - updateTime < 300 * 1000) {
	// super.getC(DocName.SMS_LOG)
	// .update(new BasicDBObject("_id", DboUtil.getLong(dbo,
	// "_id")),
	// new BasicDBObject("$set", new BasicDBObject(
	// "status", Const.STATUS_OK).append(
	// "updateTime",
	// System.currentTimeMillis())));
	// return true;
	// }
	...
	// String str; // 存放调用返回的结果,判断成功与否应该分析这个字符串
	// SmsService sms = new SmsService(); // 这里修改成你自己的用户名和密码
	// String content = "注册验证码为：" + random(6) + ",如非本人操作，请忽略！";
	// str = sms.singleSend("13269351865", content); // 这里修改成要发送的手机号码和发送内容
	// System.out.println(str);
	// // String newstring = null;
	// // try {
	// // System.out.println(str);
