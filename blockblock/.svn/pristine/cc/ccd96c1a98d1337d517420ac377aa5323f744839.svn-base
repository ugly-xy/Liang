...
//	private static String DEF_URL = "http://127.0.0.1:8545";
	 ...
	 "https://mainnet.infura.io/kF3ENaQiBKW3uQlgIWL3";
	 ...
	 * @throws IOException
	 */
	public static Map<String,BigInteger> getGas(Web3j web3j) throws IOException {
		BigInteger cbn = EthService.getCurrentBlockNumber(web3j);
		EthBlock eb = EthService.getBlockEthBlock(web3j, cbn);
		Map<String,BigInteger> gas = new HashMap<String,BigInteger>();
		BigInteger gasLimit = eb.getBlock().getGasLimit();
		System.out.println(cbn + " " +eb.getBlock().getGasUsed()+ " " + eb.getResult().getGasLimit());
		BigInteger gasPrice = eb.getBlock().getGasUsed().divide(gasLimit);
		gas.put("gasLimit", gasLimit);
		gas.put("gasPrice", gasPrice);
		return gas;
	}
	 ...
	 * @throws Exception
	 */
	public static TransactionReceipt sendTransaction(Web3j web3j, Credentials credentials, String toAddr, BigDecimal value,
			Convert.Unit unit) throws InterruptedException, IOException, TransactionException, Exception {
		return Transfer.sendFunds(web3j, credentials, toAddr, value, unit).send();
	}
