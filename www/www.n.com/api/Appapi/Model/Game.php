...
class Model_Game extends PhalApi_Model_NotORM {
	...
	public function endGame($liveuid,$gameid,$type,$ifset)
	{
		...
		if($type==2 ||$type==3)
		{
			...
			foreach($total as $k=>$v){
				$income_game=floor($v['total']*$getConfigPri['income_game']); //"游戏收益" 获得权重值(收益)
				DI()->notorm->users
					->where('id = ?', $v['uid'])
					->update(array('coin' => new NotORM_Literal("coin + {$v['total']}"),'grade' => new NotORM_Literal("grade + {$income_game}")));
				...
				if($action==4){
					...
					if($coin!=''){
						...
						foreach( $total as $k=>$v){
							...
							$win+=$gamecoin;
							$income_game=floor($gamecoin*$getConfigPri['income_game']); //"游戏收益" 获得权重值(收益)
							DI()->notorm->users
								->where('id = ?', $v['uid'])
								->update(array('coin' => new NotORM_Literal("coin + {$gamecoin}"),'grade' => new NotORM_Literal("grade + {$income_game}")));
	...
	public function gameBet($uid,$gameid,$coin,$action,$grade)
	{
		...
		/*更新用户经验值(权重值)*/
		$getConfigPri=getConfigPri();
		$expend_game=floor($coin*$getConfigPri['expend_game']); //用户房间扣费获得权重值(消费)
		DI()->notorm->users
				->where('id = ?', $uid)
				->update( array('grade' => new NotORM_Literal("grade + {$expend_game}")));
