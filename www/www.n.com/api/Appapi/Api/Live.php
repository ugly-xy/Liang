...
class Api_Live extends PhalApi_Api {
	...
	public function getRules() {
		return array(
			...
			'sendGift' => array(
				...
				'sendtype' => array('name' => 'sendtype', 'type' => 'int',  'require' => true, 'desc' => '送礼类型 0直播  1视频'),
			),
	...
	public function createRoom() {
		...
		if($configpri['level_islimit']==1){
			if( $userinfo['level'] < $configpri['level_limit'] ){
				$rs['code']=1003;
				$rs['msg']=T('等级小于{level_limit}级，不能直播',array('level_limit'=>$configpri['level_limit']));
				return $rs;
			}
		}
		...
		$info['shut_time']=$configpri['shut_time'].T('秒');
		$info['kick_time']=$configpri['kick_time'].T('秒');
	...
	public function changeLive() {
		...
		if($app_key && $master_secret && $status==1 && $info){
			...
			$title=T('你的好友：{user_nicename}正在直播，邀请你一起',array('user_nicename'=>$anthorinfo['user_nicename']));
	...
	public function checkLive() {
		...
		if($iskick>$nowtime){
			...
			$rs['code']=1004;
		$rs['msg']=T('您已被踢出房间，剩余{surplus}秒',array('surplus'=>$surplus));
		}else{
	...
	public function enterRoom() {
		...
		$userinfo['contribution'] = round($contribution);
		...
		// $contributionround($userinfo['contribution'])
        $userinfo['sign']=$userinfo['contribution'].'.'.($userinfo['level']+100).'1';
		
		// file_put_contents('./pay.txt',date('y-m-d H:i:s').' 提交参数信息 ch:'.$userinfo['sign']."\r\n",FILE_APPEND);
		...
		$liveinfo = $domain->enterRoom($liveuid);
		
		$info['pull']=$liveinfo['pull'];
        $info['stream']=$liveinfo['stream'];
	...
	public function sendGift() {
        ...
        if($gift_info['type']==2){
            ...
            if($guard_info['type']==0){
               $rs['code']=1002;
                $rs['msg']=T('该礼物是守护专属礼物奥~');
                return $rs; 
            }
        }
		
		$domain = new Domain_Live();
		$result=$domain->sendGift($uid,$liveuid,$stream,$giftid,$giftcount,$sendtype);
        ...
        $rs['info'][0]['swftime']=$result['swftime'];
    ...
    public function setShutUp() { 
        $rs = array('code' => 0, 'msg' => T('禁言成功'), 'info' => array());
	...
	public function kicking() {
		$rs = array('code' => 0, 'msg' => T('踢人成功'), 'info' => array());
		...
		if($admin_uid!=60){
			if($admin_touid==50 ){
				$rs['code']=1001;
				$rs['msg']=T('对方是主播，不能被踢出');
				return $rs;
			} 
            
            if($admin_touid==40 ){
				$rs['code']=1002;
				$rs['msg']=T('对方是管理员，不能被踢出');
				return $rs;
			}
            /* 守护 */
            $domain_guard = new Domain_Guard();
	...
	public function superStopRoom(){
		...
		$rs = array('code' => 0, 'msg' => T('关闭成功'), 'info' =>array());
