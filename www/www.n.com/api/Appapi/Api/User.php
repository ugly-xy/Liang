...
class Api_User extends PhalApi_Api {
	...
	public function getRules() {
		return array(
			...
			'getwallet' => array(
				'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '用户ID'),
                'token' => array('name' => 'token', 'type' => 'string', 'require' => true, 'desc' => '用户token'),
				'p' => array('name' => 'p', 'type' => 'int', 'min' => 1,  'desc' => '分页'),
			),
			
			'getwithdraw' => array(
				'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '用户ID'),
                'token' => array('name' => 'token', 'type' => 'string', 'require' => true, 'desc' => '用户token'),
				'p' => array('name' => 'p', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '分页'),
			),
			'addwithdraw' => array(
				'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '用户ID'),
                'token' => array('name' => 'token', 'type' => 'string', 'require' => true, 'desc' => '用户token'),
                'account' => array('name' => 'account', 'type' => 'string', 'require' => true, 'desc' => '提现账号'),
			),
			'delwithdraw' => array(
				'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '用户ID'),
                'token' => array('name' => 'token', 'type' => 'string', 'require' => true, 'desc' => '用户token'),
                'number_id' => array('name' => 'number_id', 'type' => 'string', 'require' => true, 'desc' => '提现账号id'),
			),
			'getsign' => array(
				'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '用户ID'),
                'token' => array('name' => 'token', 'type' => 'string', 'require' => true, 'desc' => '用户token'),
	...
	public function getBaseInfo() {
		...
		$info['name_coin']=$configpub['name_coin'];
	...
	public function updateAvatar() {
		...
		file_put_contents('./line.txt',date('y-m-d H:i:s').' 提交参数信息 request:'.json_encode($_FILES)."\r\n",FILE_APPEND);
	...
	public function updatePass() {
		...
		if($check== 0 ){
			$rs['code'] = 1004;
			$rs['msg'] = T('密码为8-20位数字与字母组合');
			return $rs;										
		}else if($check== 2){
			$rs['code'] = 1005;
			$rs['msg'] = T('密码不能纯数字或纯字母');
			return $rs;										
		}
	 ...
	 * @return string msg 提示信息
	 */
	public function getFollowsLists() {
		$rs = array('code' => 0, 'msg' => '', 'info' => array());
		
		$domain = new Domain_User();
		$info = $domain->getFollowsLists($this->uid,$this->touid,$this->p);
    ...
    */		
	...
	public function getPerSetting() {
        ...
        $info[]=array('id'=>'17','name'=>T('意见反馈'),'thumb'=>'' ,'href'=>get_upload_path('/index.php?g=Appapi&m=feedback&a=index'));
        $info[]=array('id'=>'15','name'=>T('修改密码'),'thumb'=>'' ,'href'=>'');
        $info[]=array('id'=>'18','name'=>T('清除缓存'),'thumb'=>'' ,'href'=>'');
        $info[]=array('id'=>'16','name'=>T('检查更新'),'thumb'=>'' ,'href'=>'');
	...
	public function addwithdraw() {
		...
		$domain = new Domain_User();
		$info = $domain->addwithdraw($uid,$account);
		if($info==1002){
			$rs['code'] = 1002;
			$rs['msg'] = T('此账号已存在!');
			return $rs;
	...
	public function delwithdraw() {
		...
		$uid=checkNull($this->uid);
        $token=checkNull($this->token);
        $number_id=checkNull($this->number_id);
		$checkToken=checkToken($uid,$token);
		if($checkToken==700){
			$rs['code'] = $checkToken;
			$rs['msg'] = T('您的登陆状态失效，请重新登陆！');
		...
		$domain = new Domain_User();
		$info = $domain->delwithdraw($uid,$number_id);
		if($info==1002){
			$rs['code'] = 1002;
			$rs['msg'] = T('此账号不存在!');
			return $rs;
	...
	public function getsign() {
		$rs = array('code' => 0, 'msg' => T('签到成功'), 'info' => array());
		$uid=checkNull($this->uid);
        $token=checkNull($this->token);
		$checkToken=checkToken($uid,$token);
		if($checkToken==700){
