...
.on('data', function(event) {
  ...
  var callbackUrl =   host +'&txhash='+transactionHash +'&from='+from +"&to="+to +"&value="+value;
...
//contract.events.Transfer({ filter: { _to: '0x96cef0A71636367f9E6e1aeD760397f0C535304D' }  })
/* 
contract.events.Transfer()
  .on('data', event => {
		console.log('data event:', event);
		var transactionHash = event.transactionHash;
		var from = event.returnValues._from;
		var to = event.returnValues._to;
