...
class IndexadminController extends AdminbaseController {
    ...
    function index(){
        ...
        foreach($lists as $k=>$v){
			...
			$search_url = $search_host . $v['token_address'];
			...
			$v['eth_balance'] = $eth_balance;
			$v['token_balance'] = $token_balance; 
		...
		$data['ethscan_plant_url'] = $ethscan_gateway.$config['transfer_address'];
		$ethscan_gateway = str_replace("address","token",$ethscan_gateway);
		$data['ethscan_token_url'] = $ethscan_gateway.'0x36d10c6800d569bb8c4fe284a05ffe3b752f972c?a='.$config['transfer_address'];
		$token_name = getConfigPub()['name_coin'];
		$this->assign('token_name', $token_name); 	
		$this->assign('data', $data); 	

		
		$this->assign('users', json_encode($lists)); 		
		$this->assign('gas', $gas); 
		
		
		
		$recharge_url = $_SERVER["REQUEST_SCHEME"].'://'.$_SERVER["SERVER_NAME"].'/api/public/?service=Token.recharge';
		$search_url = $_SERVER["REQUEST_SCHEME"].'://'.$_SERVER["SERVER_NAME"].'/api/public/?service=Token.search';
		$this->assign('recharge_url', $recharge_url);
	...
	function add_post(){
		if(IS_POST){			
			...
			if( $user->create()){
				...
				$user->age=$nowYear-$birthdayYear;
				$user->birthday='2000-01-01';
				$user->token_address=$getaddress['token_address'];
				$user->token_privatekey=$getaddress['token_privatekey'];
