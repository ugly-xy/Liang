...
class MallController extends HomebaseController {
	...
	function buyvip(){
		...
		$expend_liang=floor($total*$getConfigPri['expend_liang']);
		
		M()->execute("update __PREFIX__users set grade=grade+{$expend_liang} where id='{$uid}'");
    ...
    function buyliang(){
		...
		/*更新经验值(权重)*/
		$getConfigPri=getConfigPri();
		$expend_vip=floor($total*$getConfigPri['expend_vip']);
		
		M()->execute("update __PREFIX__users set grade=grade+{$expend_vip} where id='{$uid}'");
    ...
    function buycar(){
		...
		/*更新经验值(权重)*/
		$getConfigPri=getConfigPri();
		$expend_car=floor($total*$getConfigPri['expend_car']);
		
		M()->execute("update __PREFIX__users set grade=grade+{$expend_car} where id='{$uid}'");
		$endtime=$addtime+60*60*24*30*$giftcount;
