...
class ContributeController extends HomebaseController {
	...
	function index(){
		...
		foreach($list_day as $k=>$v){
			...
			$list_day[$k]['total']=round($v['total']);
		}
        ...
        $list_day_total2=round($list_day_total);
        $this->assign("list_day_total",$list_day_total2);
		...
		foreach($list_week as $k=>$v){
			...
			$list_week[$k]['total']=round($v['total']);
		}
		...
		$list_week_total2=round($list_week_total);
        $this->assign("list_week_total",$list_week_total2);
		...
		foreach($list_month as $k=>$v){
			...
			$list_month[$k]['total']=round($v['total']);
		}
		...
		$list_month_total2=round($list_month_total);
        $this->assign("list_month_total",$list_month_total2);
		...
		foreach($list_all as $k=>$v){
			...
			$list_all[$k]['total']=round($v['total']);
		}
		...
		$list_all_total2=round($list_all_total);
        $this->assign("list_all_total",$list_all_total2);
