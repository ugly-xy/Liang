...
class HomebaseController extends AppframeController {
	...
	function _initialize() {
		...
		if($_REQUEST['language']!=''){
			$language_type=$_REQUEST['language'];
		}else{
			$language_type='zh_cn';
		}
		
		if($language_type=='ko'){
			$language=include(SITE_PATH.'language/ko.php');
		}else{
			$language=include(SITE_PATH.'language/zh_cn.php');
		}
		define('LANGUAGE', $language);
		define('LANGUAGE_TYPE', $language_type);
		$this->assign("language",json_encode($language) );
		$this->assign("language_type",$language_type );
