...
class IndexController extends HomebaseController {
	...
	function settlement(){
		...
		// file_put_contents('./logs.txt', $today,FILE_APPEND);
		//今天总款
		$total=M('platform_total_weight')->where("addtime={$today}")->find();
		//所有用户今天的总权重值
		$weight=M('users_total_weight')->where("addtime={$today}")->sum('weight');
		//当前用户的数据
		$list=M('users_total_weight')->where("addtime={$today}")->select();
		
		if(!$total || !$weight ||!$list){
			$rs['code']=1001;
			$info=array(
				'total'=>$total,
				'weight'=>$weight,
		...
		foreach($list as $k=>$v){
            ...
            $zhi=$total['totalweight']*$v['weight']/$weight; //该用户今天可得
			
			$value=number_format($zhi,4); //保留四位小数
			
			M('users')->where("id={$v['uid']}")->setInc('mine',$value);
			M('users')->where("id={$v['uid']}")->setInc('coin',$value);
			M("users_coinrecord")->add(array("type"=>'income',"action"=>'mining',"uid"=>$v['uid'],"touid"=>$v['uid'],"totalcoin"=>$value,"addtime"=>time()));	
