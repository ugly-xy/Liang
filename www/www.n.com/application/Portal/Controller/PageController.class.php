...
class PageController extends HomebaseController{
	...
	public function lists() {
		...
		$page=M("posts")->field("id,post_title,post_title_kr")->where("type='2'")->order("orderno asc")->select();
		foreach ($page as $k => $v) {

			if(LANGUAGE_TYPE=='ko'){
				$page[$k]['post_title']=$v['post_title_kr'];
			}
		}
	...
	public function newslists() {
		...
		$list=M("term_relationships")
					->alias("a")
					->field("b.id,b.post_title,b.post_title_kr")
					->join(C ( 'DB_PREFIX' )."posts b ON a.object_id = b.id")
		...
		foreach ($list as $k => $v) {
			...
			if(LANGUAGE_TYPE=='ko'){
				$list[$k]['post_title']=$v['post_title_kr'];
			}
		}
	...
	public function news() {
		$id=$_GET['id'];
		$news=M("posts")->field("post_title,post_content,post_title_kr")->where("id='$id'")->find();
		if(LANGUAGE_TYPE=='ko'){
			$news['post_title']=$news['post_title_kr'];
		}
   ...
   public function test11111111111111111() {
	...
	public function questions(){
		...
		//读取常见问题下的文章列表
		$list=M("term_relationships")->order("listorder")->where("term_id=3")->select();
		$questionList=array();
		foreach ($list as $k => $v) {
