...
var language={
	'zh_cn':'直播内容包含任何低俗、暴露和涉黄内容，账号会被封禁；安全部门会24小时巡查哦～',
	'ko':'타인 비방과 비하, 정치성향 및 선정적인 대화, 청소년 보호법 위반, 불법 등의 채팅은 제재의 대상이 됩니다',
};
			...
			if(error){
				...
				if(res != null){
					...
					if(userInfo['id'] == data.uid ){
						...
						socket.level_anchor = userInfo['level'];
						...
						var languageType=userInfo.language;
						sendSystemMsg(socket,language[languageType]);
						return;
					}else{
						socket.disconnect();
					}
