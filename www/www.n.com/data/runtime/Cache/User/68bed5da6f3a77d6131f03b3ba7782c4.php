...
<body>
	<div class="wrap">
		...
		<form class="form-horizontal js-ajax-form" action="<?php echo U('indexadmin/edit_post');?>" method="post">
			<fieldset>
			   <input type="hidden" name="id" value="<?php echo ($userinfo['id']); ?>" id="id"/>
			   <?php if($_SESSION["ADMIN_ID"] == '1'){ ?>
				<div class="control-group">
					<label class="control-label">用户名</label>
					<div class="controls">
						<input type="text" name="user_login" value="<?php echo ($userinfo['user_login']); ?>" id="user_login" readonly/>
				...
				<div class="control-group">
					...
					<div class="controls">
						<input type="text" name="consumption" value="<?php echo ($userinfo['consumption']); ?>" id="consumption"/> 控制用户等级（消费总额）
						<span class="form-required">*</span>
					</div>
				</div>
				<div class="control-group">
				...
				<div class="control-group">
					...
					<div class="controls">
						<label class="radio inline" for="active_true"><input type="radio" name="user_status" value="1" <?php if($userinfo['user_status'] == '1'): ?>checked<?php endif; ?> id="active_true" /><?php echo L('ENABLED');?></label>
						<label class="radio inline" for="active_false"><input type="radio" name="user_status" value="0" <?php if($userinfo['user_status'] == '0'): ?>checked<?php endif; ?> id="active_false"><?php echo L('DISABLED');?></label>
					</div>
				</div>
				<div class="control-group">
					<label class="control-label">是否为视频发布者</label>
					<div class="controls">
						<label class="radio inline" ><input type="radio" name="is_ad" value="0" <?php if($userinfo['is_ad'] == '0'): ?>checked<?php endif; ?> />否</label>
						<label class="radio inline" ><input type="radio" name="is_ad" value="1" <?php if($userinfo['is_ad'] == '1'): ?>checked<?php endif; ?> >是</label>
					</div>
				</div>	
			</fieldset>
